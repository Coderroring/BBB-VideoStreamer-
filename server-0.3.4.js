// ==============================================
// BBB-VideoStreamer v0.3.4 (状态管理修复版)
// Author: Coderroring
// Features:
//  - 修复 FFmpeg faststart 导致的播放链接提前出现问题 (严格状态管理)
...
app.get('/download', async (req, res) => {
    ...
    // 如果文件存在但缓存状态不完整，可能是服务器重启导致的孤立文件。
    // 在这里我们强制清除映射，让它重新下载/转码，以确保文件完整性（或者直接在status页处理孤立文件）
    ...
    ytdlp.on('close', code => {
        ...
        const ffmpegArgs = [
            ...
            '-ar', '44100',
            '-b:a', '96k',
            '-movflags', '+faststart', // 保持 faststart
            '-y',
            outputPathFinal
        ];
        ...
        ffmpeg.on('close', c => {
            // **此回调函数是 FFmpeg 真正完成所有操作（包括 faststart）后才执行的**
...
app.get('/list', (req, res) => {
    ...
    if (cachedItems.length > 0) {
        listHtml = cachedItems.map(([videoId, cacheData]) => {
            if (cacheData.status === 'completed' && fs.existsSync(cacheData.filePath)) {
                 ...
                 return `<li>${cacheData.title || '正在处理的视频'} (处理中)</li>`;
            } else if (cacheData.status === 'failed') {
                 return `<li>${cacheData.title || '失败的视频'} (失败 - ${cacheData.error})</li>`;
            }
            return '';
        }).join('');
    }

    // 额外的目录扫描，以防服务器重启后 cacheMap 完全丢失
    const filesInDir = fs.readdirSync(cacheDir).filter(f => f.endsWith('_final.mp4'));
     if (filesInDir.length > 0) {
         ...
         filesInDir.forEach(f => {
              ...
              if (!cacheMap.has(videoIdFromFile) || cacheMap.get(videoIdFromFile).status !== 'completed') {
                   const defaultTitle = f.replace(/^[a-f0-9]+_/, '').replace('_final.mp4', '') || f;
                   // 尝试重建 map，以在 list 页面中显示
                   if (!cacheMap.has(videoIdFromFile)) {
                        cacheMap.set(videoIdFromFile, { filePath: path.join(cacheDir, f), title: defaultTitle + " (已缓存)", url: "未知URL", status: 'completed' });
                   }
                   dirListHtml += `<li><a href="/watch?file=${f}">${defaultTitle} (已缓存)</a></li>`;
              }
         });
         // 为了避免重复，我们只显示 cacheMap 中状态为 completed 的项，并确保 dirListHtml 不重复
         if(listHtml.length === 0) { // 仅在 listHtml 为空时显示从目录加载的列表
             listHtml = dirListHtml;
         }
     }
...
app.listen(port, '0.0.0.0', () => {
    ...
    console.log(`=======================================================`);
    console.log(`  BBB-VideoStreamer v0.3.4 (状态管理修复版)`);
    console.log(`  服务器正在运行!`);
