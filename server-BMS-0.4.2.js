// ==============================================
// BBB-VideoStreamer v0.4.2 (Bug修复版)
// Author: Coderroring
// Features:
//  - v0.4.2: 修复 yt-dlp 不支持 --bili-api-extra-params 的崩溃问题
//          (回退到使用 ?p=... URL参数来选择分P)
//  - v0.4.1: PGC(番剧)支持, 单个缓存删除, URL跳转详情页
...
app.get('/download_task', async (req, res) => {
    // type=ugc 或 type=pgc
    const { bvid, aid, cid, title, ep_id, type, page } = req.query; // 新增 page
    ...
    // PGC (番剧) 使用 EPID
    // UGC (普通视频) 使用 BVID/AID + CID (因为不同P的CID不同)
    ...
    if (type === 'ugc') {
        ...
        // **修复**：通过 URL 传递分P，而不是使用 --bili-api-extra-params
        ...
        const ytArgs = [
            ...
            '--no-playlist',
            // **移除**: --bili-api-extra-params (因为它不受支持)
            '-o', outputPathRaw,
            '--socket-timeout', '30',
            '--retries', '5',
            videoUrl // URL 现在包含了 ?p=...
        ];
...
app.listen(port, '0.0.0.0', () => {
    ...
    console.log(`=======================================================`);
    console.log(`  BBB-VideoStreamer v0.4.2 (Bug修复版)`);
    console.log(`  服务器正在运行!`);
